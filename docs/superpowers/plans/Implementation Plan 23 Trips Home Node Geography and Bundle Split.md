...
**Status: COMPLETE + DEPLOYED — implemented + deployed 2026-07-24 (impl d68a986; prod at 7801cf4, container `trippy-trippy-1`, `:6768`, no migration). Independent of Tracks A/B/C; not folded into Track B.** Infra-verified post-deploy: health `{status:ok, db:connected}`, clean startup, both F-1 copy strings present in the deployed `index-*.js` bundle, Track C WebP unregressed (200). Owner prod browser QA pending (see the consolidated click-script below — this deploy is also the natural trigger to close the two outstanding Track B/C SW-upgrade owner checks).** Presentation-only across the board: no migration, no backend logic change, no SW caching change, no offline-data/write architecture. Transport-vs-presentation split shipped as designed: `request()` stamps `code: NETWORK_ERROR`/`TIMEOUT`, pure `frontend/src/utils/apiError.js` (`friendlyError`) owns wording, AuthContext (setup/login/register) + TripsHomePage route through it while rethrowing the original error (code/status preserved). Trips home renders a recoverable block (friendly copy + Try again) and hides `+ New Trip` while errored; empty state stays distinct from failed load. Frontend 240 tests + build green; backend 676 tests green (untouched, proven). Browser-verified at 375px (login/register offline → uppercased auth copy; Trips home offline → recoverable block + working Try-again recovery) and desktop (login/register offline auth copy); Setup-offline and desktop-Trips-home covered by unit tests + responsive/markup equivalence (see QA note).
