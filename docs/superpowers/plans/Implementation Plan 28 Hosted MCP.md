...
- **I-28-A.** Whether Claude Code's and Codex CLI's *current* builds speak 2026-07-28 or a 2025 revision is unknown. W0 turns this into a fact (D-28-2 names both as must-work).
...
**Codex CLI** — `~/.codex/config.toml`, an `[mcp_servers.trippy]` table with `url = "https://trippy.zyroi.com/mcp"` and the bearer supplied through the installed version's header/env mechanism. **The exact key names are recorded here by W0 after reading the installed version's docs — do not guess them.**

**Owner's bot (raw)** — `POST https://trippy.zyroi.com/mcp` with `Authorization: Bearer trp_…`, `Content-Type: application/json`, `Accept: application/json, text/event-stream`, `MCP-Protocol-Version: 2026-07-28`, `Mcp-Method: tools/call`, `Mcp-Name: list_trips`, body `{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"list_trips","arguments":{}}}`. Uploads: `curl -T file.png -H "Content-Type: image/png" <uploadUrl>` — no bearer on the upload.
