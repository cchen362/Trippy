...
**Status:** 2026-09-16 — **W3 COMPLETE LOCALLY (not deployed; Deploy B = W3 + W5)** — see the W3 status line; highest applied migration locally is **035**, production is still at 034. Previous: **DEPLOY A DONE — W1 + W2 DEPLOYED** at `f029367` (server pulled `69cdc86` then the timing-seam commit; migrations 033 + 034 applied at container boot, `_migrations` rows 33/34; backup `backups/trippy-pre-plan28A-20260915-213101.db`, integrity ok; `.env` gained `MCP_ENABLED=1` / `MCP_PUBLIC_URL`). Production-only gates **passed** — see the W2 status line. Owner Appendix A §A + §B1–6 outcome recorded there. `/mcp` with `list_trips` / `get_trip` / `prepare_draft` / `apply_draft` / `get_apply_status`, migrations **033** `integration_tokens` and **034** `mcp_drafts`, `/api/integrations/tokens`, and the Integrations panel exist on `main`; `mcp_upload_tickets`, `request_upload_ticket`, `prepare_delete`, and `newTrip` targets exist on `main` since W3 but are **not yet in production**. Highest applied migration in production is **034**. Written at `c8e8f1c` from the settled planning recommendation; all ten owner rulings D-28-1 … D-28-10 are taken, so no wave is blocked on a product question. One plan-level interpretation (I-28-2) and two non-blocking questions (Q-28-1, Q-28-2) are recorded below with the default each wave will follow unless the owner says otherwise; I-28-1 (Integrations UI in the Account modal) was confirmed by the owner the same day. Waves are implemented later via `/implement-milestone`; each wave updates its own status line and commits. **W0 (the SDK v2 handshake spike) completed 2026-09-16 with a GO — see its status line; it merged no code.**
...
| `multi_booking_not_available` | blocker (**W2 only**) | `bookings.length > 1` while the write path inserts exactly one; W3.2 retires it |
...
**Status:** **DEPLOYED 2026-09-16 (Deploy A, with W2)** at `f029367`. Deploy-gate proof through the public host: `POST /mcp` → 401 with `WWW-Authenticate … resource_metadata=` and a `CF-RAY` header (tunnel passes `/mcp`, no bot challenge — F-28-14 (a)(d)); `GET /mcp` → 405; `/.well-known/oauth-protected-resource` and the `/mcp`-suffixed variant → JSON; `/api/health` → `mcp.enabled: true`, `protocolVersions[0] = 2026-07-28`. Local completion facts follow. Migration **033** `integration_tokens` applied locally (`_migrations` id 33). Suites: backend **39 files / 875 tests** (baseline 35 / 831 → +4 files, +44 tests: `migration033`, `mcpTokens`, `mcpTools`, `mcpClient.e2e`), frontend **44 / 305** (+1 file, +4 tests), `npm run build` green, `git diff --check` clean, `npm audit` unchanged at 14 pre-existing. Dependencies added: `@modelcontextprotocol/server|node|express@2.0.0` (exact) and `@modelcontextprotocol/client@2.0.0` (dev, for the e2e test). `.claude/launch.json` backend entry now carries `MCP_ENABLED=1` / `MCP_PUBLIC_URL=http://localhost:3002/mcp`; `.env.example` documents both.

**Verified locally, by hand:** rollback gate (`MCP_ENABLED` unset → `POST /mcp` 404, `/.well-known/oauth-protected-resource` 404, `/api/health.mcp.enabled === false`); enabled path (`POST /mcp` no token → 401 + `WWW-Authenticate … resource_metadata="…/.well-known/oauth-protected-resource/mcp"`, `GET /mcp` → 405 `Allow: POST`, metadata document served at both well-known paths); Integrations panel in Chrome at 455 px (Chrome's floor; layout is 375-first) and 1280 px — Account → Integrations → empty state → New token (`qa-phone`, `trips:read`, 30 days) → one-time reveal → Copy ("Copied") → Done → list shows `trp_BHY318Y2…` prefix, scope pill, created/expires/last-used line → Revoke → Confirm? → row dims to "revoked Sep 16, 2026"; **MCP Inspector 2.6.0** `--cli` `tools/list`, `list_trips(includePast)`, `get_trip(include days+bookings)` all served; **real Claude Code** from a fresh owner-opened desktop session listed all 5 trips, fetched the Ipoh trip's 4 days / 4 bookings, and refused the "add a booking" ask by reporting only two read-only tools exist (no crash, no silent success); `last_used_at` stamped on the token row; after revocation the next raw call was 401 and `claude mcp list` reported "Server rejected the configured Authorization header (HTTP 401)". Registration removed afterwards.

**Deviations from the W1.4 text, all engineering, none product:** (1) the bearer verifier is a ~30-line middleware in `routes/mcp.js`, not the SDK's `requireBearerAuth` — that helper hard-rejects any `AuthInfo` without `expiresAt`, and Q-28-1's "no expiry" tokens are a product choice; `req.auth` is still the seam `toNodeHandler` forwards as `authInfo`. (2) The RFC 9728 document is served by hand, not via `mcpAuthMetadataRouter` — that router requires RFC 8414 authorization-server metadata with an HTTPS issuer, and there is no AS (D-28-1); the document omits the optional `authorization_servers` field rather than advertise a fake one. (3) The per-IP limiter runs **inside the 401 branch** of the bearer check rather than as a mounted middleware ahead of it: `express-rate-limit` blocks on the counter *before* the request runs, so `skipSuccessfulRequests` alone would still have locked out a valid token from an IP whose 401 budget was spent (a household NAT problem) — the e2e test caught this. (4) Scope failures are returned as an `isError` tool result `{ error: 'insufficient_scope', requiredScope }` rather than an HTTP 403, so an SDK client's session survives and the model can tell the user which scope is missing (Appendix A §A3's expectation); an HTTP-level 403 mid-session is a transport error to every client. (5) Revoked tokens stay in the list dimmed with `revokedAt` rather than vanishing, so Appendix A §A5's "both tokens still listed" reads literally.

**Open for W2+:** no per-call tool log yet (W5.2); `MCP_PUBLIC_URL` absence with the flag on throws at boot by design. Legacy dev data shows `Singapore|MY` in `destinationsGeo` — pre-existing Plan 23 legacy-data class, not a W1 defect.
...
- W1.4 `routes/mcp.js`: `express.json({ limit: '1mb' })` local to the router; Origin check (absent → ok; present and ≠ `FRONTEND_URL` → 403); `requireBearerAuth` with a verifier that wraps W1.1 and returns `authInfo = { token, clientId: token.id, scopes, expiresAt, extra: { userId } }` (**stamp D-28-1**); one `createMcpHandler(factory, { legacy: 'stateless' })` wrapped by `toNodeHandler` and called as `(req, res, req.body)` (**W0 finding — not the README's per-request `NodeStreamableHTTPServerTransport`, which is the legacy idiom the fallback builds internally**); `GET/DELETE → 405`; ignore `Mcp-Session-Id` / `Last-Event-ID`; `X-Accel-Buffering: no`; `mcpAuthMetadataRouter` serving `/.well-known/oauth-protected-resource` with `resource = MCP_PUBLIC_URL` and the three scopes. Mounted in `index.js` **before** the global JSON parser and only when `config.mcpEnabled`.
...
- W1.6 Rate limits: `mcpTokenLimiter` 120 req/min keyed by token id (after verification); `mcpAnonLimiter` 30 req/min keyed by IP on the 401 path. Health: `mcp: { enabled, protocolVersions: ['2026-07-28', ...SUPPORTED_PROTOCOL_VERSIONS] }` (F-28-23; the exported constant is the legacy list only — W0 finding).
...
**Status:** **DEPLOYED 2026-09-16 (Deploy A, with W1)** at `f029367`. **Cloudflare keep-alive gate PASSED through `https://trippy.zyroi.com/mcp`** (F-28-14 (b)): with the owner-approved test seam `MCP_APPLY_RESOLVE_DELAY_MS=150000` armed on the server, a curl `apply_draft` on the verify trip streamed `notifications/message` at +1 s, `: keepalive` every 15 s, and the result at **+153 s** — no 524. **Retry-after-timeout PASSED**: the client was killed at +40 s mid-resolve; the server finished the apply on its own, `get_apply_status` returned the stored `applied` result with the booking id, and a second `apply_draft` answered `already_applied` with the **same** id (two drafts → exactly two bookings in the prod DB). Seam disarmed afterwards (`.env` line removed, container recreated, `/api/health` ok). Note: the kill landed *during* the resolve phase, not after the last per-booking progress line — with one booking the write follows that line within milliseconds, so the stricter cut is only reachable in W3's N>1 apply. **Owner Appendix A §A1–A5 and §B1–6 PASSED on the public host 2026-09-16** (§A run in Claude Code with `claude mcp add --header`; §B1/B2 screenshot→preview→apply with the stop visible in Logistics and Map, §B3 duplicate-ref warning fired and applying anyway produced the second booking as specified, §B4 `outside_trip_dates` refused a Singapore hotel against a Kyoto trip, §B5 a hand-added booking made the pending draft stale and forced a re-prepare, §B6 `already_applied` returned the same booking id). **QA-technique finding (F-28-27): a conversational client can satisfy a repeat request from its own memory and never call the tool** — asking for the same screenshot twice in one session made Claude answer "I already added that" without a second `prepare_draft`, so the duplicate check never ran; §B3 only exercised the server once re-run in a **fresh session**. Read `mcp_drafts` and the DB, not the chat reply, when scoring an MCP QA step. Local completion facts follow. Migration **034** `mcp_drafts` applied locally (`_migrations` id 34). Suites: backend **42 files / 907 tests** (W1 baseline 39 / 875 → +3 files, +32 tests: `migration034`, `bookingStopSplit`, `mcpDrafts`, plus extensions to `mcpClient.e2e` and `mcpTools`), frontend untouched (44 / 305 stands — W2 changes no frontend file), `git diff --check` clean. No new dependency (`geo-tz` and `fromJsonSchema` were already in place).

**Verified locally, by hand, against real rows in the dev DB:** MCP Inspector 2.6.0 `--cli` (2025-11-25 wire, no progress token) `prepare_draft` → preview with `timezone_unknown` info and `willCreate: true` → `apply_draft` (4.4 s: real Nominatim + Unsplash) → one `bookings` row, one `stops` row on 2026-11-10 at Kyoto Station coordinates with a photo → second `apply_draft` → `already_applied`, same booking id, still one row → `get_apply_status` by `idempotencyKey` → the stored result. **Real Claude Code** (fresh owner-opened desktop session, registration via `claude mcp add`, removed afterwards): a real hotel screenshot (Singapore, Sep 25–26) → blocked `outside_trip_dates`, nothing written, model reported it could not force it; duplicate ref → `duplicate_confirmation_ref` warning carrying the existing id, not applied; out-of-range hotel → blocker; **stale**: draft prepared → owner added a booking by hand in the PWA → `apply_draft` → `stale`, model re-prepared and applied, then "apply it again" → `already_applied` with the same id. PWA (Chrome extension, logged-in tab): both MCP-created hotels render on the right days in Plan with photos and in Logistics with their confirmation ref, identical to the hand-added one. Token revoked afterwards → next raw call 401. Verify trip `MCP QA` (`20b95db590c4048d2157748c506b550d`) left for the owner to delete from the UI.

**Deviations from the W2 text, engineering not product:** (1) **Keep-alive design.** `apply_draft` sends one notification *before* the first provider call, which is what flips the SDK's `auto` response to SSE and starts its 15 s comment keep-alives; `notifications/progress` when the client supplied a `progressToken` (the spec forbids it otherwise), else a `notifications/message` info line — which required declaring `capabilities.logging` on the `McpServer`. The plan's "keep-alive comment every 20 s" is therefore the SDK's own timer, not hand-written; the e2e test proves the token-less raw-fetch path comes back `text/event-stream`. (2) **One new issue code, W2-only:** `multi_booking_not_available` (blocker) for `bookings.length > 1`, because the write path inserts exactly one; `multi_leg_detected` still fires as the info signal. W3 retires the blocker when N > 1 lands. (3) `prepare_draft` output carries `draftStatus` (additive) so a re-sent idempotency key that returns an already-applied/expired draft is legible. (4) `sourceDocument.status` is `not_requested` only for `source.kind = 'manual'`; screenshot/pdf/email_text report `unsupported` until W3's tickets exist. (5) Re-validation at apply runs over the stored *normalized* bookings, so blockers that lived only on the raw input (`cost_not_accepted`) are carried forward from the draft's stored issues and refused alongside anything the live re-check finds; such a draft is marked `invalid`. (6) `bookings.js` now exports `defaultShowInItinerary` as well as `writeBookingRow`/`formatBooking`, so the preview's "will a stop be created" answer and the insert share one rule. (7) The `timezone_unknown` suggestion is best-effort and free: `detailsJson.lat/lng` → `geo-tz`, else `resolvePlace({ allowNetwork: false })` (curated table + `place_resolution_cache`, no fetch), else IATA → city → the same cache-only lookup — in QA every plain city name missed the cache, so no suggestion was offered; D-28-10 says "when available" and that stands.

**Open for W3+:** `prepare_draft`'s `bookings.items` schema was tightened to a full `BookingInput` property schema after Claude Code's first screenshot call sent the wrong argument shape (validation caught it; the model self-corrected) — re-check on the first W3 real-client run that first-call shape errors are gone. W2.2's split leaves `syncStopWithBooking` as `writeBookingStop(await resolveBookingStopData(b))`; `updateBooking`'s existing-stop path is unchanged and covered by `locationIntegration.test.js`.
...
**Status:** **COMPLETE LOCALLY 2026-09-16 — NOT DEPLOYED** (owner ruling: Deploy B ships W3 + W5 together). Migration **035** `035_mcp_upload_tickets.js` applied locally (`_migrations` id 35) and **proven on a copy of the dev DB with two seeded pre-existing attachment BLOBs** — both rows' `content_hash` matched an independent sha256 of their `content`, `source` defaulted `'manual'`, `integrity_check ok`. Suites: backend **44 files / 951 tests** (W2 baseline 42 / 907 → +2 files, +44 tests: `migration035`, `mcpUploads`, plus 18 new cases in `mcpDrafts` and the 7-tool `tools/list` assertion in `mcpTools` + `mcpClient.e2e`), frontend **44 / 305** untouched (W3 changes no frontend file — attachments already rendered in Logistics via `documents[]`), `npm run build` green, `git diff --check` clean. No new dependency, no env change. `/mcp` now serves seven tools: the five from Deploy A plus `request_upload_ticket` and `prepare_delete`; `PUT /mcp/uploads/:ticket` is mounted on the same router with its own `express.raw({ limit: '10mb' })`, no bearer, no Origin check.

**Validation gates — all PASSED locally, scored from `mcp_drafts` / `mcp_upload_tickets` / the DB, never from the chat reply (F-28-27), each in a fresh `claude -p` session against `.claude/launch.json`'s backend :3002 (the CLI is logged in on this machine now, so W0's "owner must open a desktop session" workaround was not needed):** (1) **real Claude Code**, two-leg Peach itinerary PNG (`PCH4Q2`, NRT→KIX 10 Nov / KIX→NRT 12 Nov, inside the `MCP QA` verify trip) → one draft, issues `multi_leg_detected` + 4× `timezone_unknown` (all info), `plannedEffects` `willCreate` on both dates → apply → **two** `bookings` rows sharing the ref and **two** `transit` stops on 2026-11-10 / 2026-11-12 (leg 1 resolved to KIX coordinates; leg 2 unresolved by live Nominatim for "Tokyo Narita (NRT) Terminal 1" — resolver quality, identical to the UI path, not W3). (2) **real Claude Code with Bash**: the host ran `sha256sum` + `wc -c`, called `request_upload_ticket` (its first call sent a 63-char hash — `invalid_sha256` refused it and the model self-corrected), then `curl -T two-leg.png -H "Content-Type: image/png" <uploadUrl>` → **201** `{ attachmentId, alreadyAttached:false }`; the identical curl again to the same ticket URL → **200, the same attachmentId, `alreadyAttached:true`**, still one `booking_attachments` row (`source='mcp'`, hash = the file's sha256), ticket `used`; in the PWA (Chrome extension, logged-in tab) the MM 113 card shows the paperclip, its modal lists **Photo 1** under Documents, and opening it renders the exact uploaded PNG in DocumentViewer. (3) **§B9**: "create `MCP New` 2027-03-01→04 in Osaka + hotel" → `createdTrip:true`, one `trips` row, **4** `days` rows seeded `Osaka`/`JP`, one `trip_scopes` row, the booking, and a hotel stop on 2027-03-02 at real Osaka coordinates with an Unsplash photo (the provisional-day country bias reached the resolver); Trips Home shows the card with an OSAKA single-node cover and "In 6 months". The same request **without dates** → two `new_trip_invalid` blockers (`newTrip.startDate`, `newTrip.endDate`), `applyAllowed:false`, **zero** `trips` rows named `MCP Nodates` (D-28-5). (4) **§B10**: a ¥4,200 cost logged through the Expenses UI against MM 113 → `prepare_delete` preview: `linkedStop.effect willDelete`, `linkedExpenses[0].effect willUnlink`, summary "…1 linked cost (JPY 4,200) will be kept and unlinked" → apply → booking row and stop row gone, the `expenses` row present with `booking_id = NULL`, and the entry still listed in Expenses ("MM 113 · ¥4,200"). (5) **Codex CLI 0.144.6** (D-28-2's fourth client, matrix row "W1 or W3 at latest"): `bearer_token_env_var` + `default_tools_approval_mode = "approve"` appended to `config.toml` (backed up and restored byte-for-byte afterwards), `codex exec` completed `list_trips` and `get_trip(days, bookings)` on `MCP New`. QA token `w3-qa` revoked and the `trippy` registration removed from `~/.claude.json` afterwards; the copy DB lives only in the session scratchpad. Left in the dev DB for the owner: `MCP New` (trip `8b315ecb…`), the MM 114 booking + stop and the unlinked ¥4,200 cost on `MCP QA`.

**Deviations from the W3 text, all engineering, none product:** (1) **Provisional day for a new trip** — the plan's "createTrip inside the transaction so days exist before `writeBookingStop`" is true for the *write* phase, but `resolveBookingStopData` also needs a day during the *resolve* phase (day lookup, `getDayGeo` bias, photo city). W3.1 therefore gave it an optional `{ day }` argument: `undefined` = DB lookup (every existing caller, unchanged), an object = a provisional `{ id:null, trip_id:null, date, city, city_country }` built from `newTrip.destinations[0]`, `null` = "no day" (out of the proposed range → `no_day_for_date`). In the write transaction apply.js swaps in the real `days` row after `createTrip` (the same wiring point that already sets `resolved.booking`). The only other `stops.js` change is one extra term on the geocoder's country-bias chain, `… ?? dayGeo.countryCode ?? day.city_country` — a no-op for a real day (deriveDayGeo's layer 5 already yields the seeded `city_country`), and the only country evidence for a provisional day. The first Sonnet draft had instead changed the *stored* `countryCode` of every resolved stop in `applyResolutionFields` (an app-path behaviour change) — reverted by the orchestrator before commit; `deriveDayGeo` and D-25-1 untouched. (2) **`source` values are `'manual'` and `'mcp'` only** — the importer never writes `booking_attachments` (its files live in `import_artifact_files`, surfaced by `services/documents.js`), so the plan's `'import'` value has no writer and none was invented. (3) **Ticket media type is a class, not an exact type**: a screenshot ticket defaults to `image/png` when the client omits `mediaType`, and the PUT accepts any whitelisted *image* Content-Type for it (pdf-class likewise), storing the type actually sent; `max_bytes` is by class (5 MB image / 10 MB pdf). (4) **Idempotent re-upload works on the same ticket URL** — the gate's "the same curl again returns the same attachment id" implies the same URL, so a body whose hash already exists on the booking returns 200 + the existing id whether the ticket is `pending` or already `used`; a `used` ticket whose attachment was since deleted in the UI → 410 `ticket_used`. (5) **A ticket that cannot be issued never blocks the booking**: at apply, a coded `issueUploadTicket` failure (`too_large`, `attachment_limit`, …) yields `sourceDocument: { status:'failed', reason }` and the bookings still commit (owner boundary: "a structured booking may still be saved when [the document] cannot, and the result must say so"); screenshot/pdf **without** a sha256 → `{ status:'unsupported', reason:'sha256_required' }`; a *malformed* sha256 is a prepare-time blocker (`missing_required_field` on `source.sha256`) since the host can fix it cheaply. `source.sourceBookingIndex` (default 0) picks which of N bookings the document belongs to. No new issue code was added; the W2-only `multi_booking_not_available` is retired. (6) `sourceDocument` is stored frozen in `result_json` (ticket id, expiry, max bytes, required hash — never the URL) and **presented live**: `apply_draft` (incl. `already_applied`) and `get_apply_status` re-read the ticket row, so a `pending_upload` becomes `saved` + `attachmentId` after the PUT or `failed` (`ticket_expired`) after 15 min; `uploadUrl` is derived from `MCP_PUBLIC_URL` at read time. (7) **`prepare_delete` on an inaccessible booking** returns the `not_found` tool result and creates **no** draft (there is no trip to file it under); `expense_not_linked` (any id not among the booking's linked expenses, whether unknown, another trip's, or another booking's — never distinguished) is a stored blocker. Its fingerprint is `sha256(computeBookingFingerprint(trip) :: linked stop ids :: linked expense ids)` per the Schema text, so a cost added or removed after preview goes stale. The apply result reports every `booking_required` stop `deleteBooking` removes (`requiredStopIds`), not only the headline `linkedStop`. (8) `eachDate` (trips.js) and `fetchOwedRows` (expenses.js) are now exported; a backend `formatMinor` ("JPY 4,200", ISO code, no symbol table) was added to `utils/currency.js` for the delete summary. (9) `attachments.js` was split into `writeAttachment` (shared, no access check, computes the hash) used by both `addAttachment` (UI, `'manual'`) and the ticket path (`'mcp'`); REST behaviour and `attachments.test.js` unchanged; `listAttachments` additionally returns `contentHash`/`source`.

**Open for W5 / Deploy B:** re-run Appendix A §B7–10 and §C through the public host after Deploy B, including the upload (`curl -T` from the owner's machine); the pre-deploy backup is mandatory (035 rewrites every `booking_attachments` row). Not seen this wave: any first-call argument-shape error from Claude Code on `prepare_draft` (the W2 open item) — the two-leg and newTrip calls were shaped correctly on the first try.
...
**Status:** NOT STARTED. Depends on W3 complete locally (done 2026-09-16); ships with W3 as Deploy B.
...
**Owner ruling 2026-09-16 (supersedes the per-wave cadence below): two deploys, not three.** **Deploy A = W1 + W2 together**, once W2 is complete locally — that is the first moment the Cloudflare 100 s keep-alive test (production-only) can run, and it runs *before* W3 builds its multi-booking apply on the same streaming path. **Deploy B = W3 + W5 together** at the end (W5 is docs + SIGTERM, so W3's row-rewriting migration still effectively has the release to itself and takes its pre-deploy backup). W1 is not deployed alone (flag-gated, read-only, nothing time-sensitive). Consequences: W2's "Depends on W1 deployed" reads as *W1 complete locally*; W1's deploy gate and Appendix A §A run as part of Deploy A; W3's "Depends on W2 deployed" stands as written. The release descriptions below remain each wave's local QA checklist.
