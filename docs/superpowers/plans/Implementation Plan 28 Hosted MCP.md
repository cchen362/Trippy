...
**Status:** **COMPLETE LOCALLY — 2026-09-16. Not yet deployed** (Deploy A = W1 + W2, owner-triggered through the `deploy` skill; the Cloudflare ≥ 120 s keep-alive test and Appendix A §B1–6 run then). Migration **034** `mcp_drafts` applied locally (`_migrations` id 34). Suites: backend **42 files / 907 tests** (W1 baseline 39 / 875 → +3 files, +32 tests: `migration034`, `bookingStopSplit`, `mcpDrafts`, plus extensions to `mcpClient.e2e` and `mcpTools`), frontend untouched (44 / 305 stands — W2 changes no frontend file), `git diff --check` clean. No new dependency (`geo-tz` and `fromJsonSchema` were already in place).
