...
> **All four items below were picked up by [Implementation Plan 27](Implementation%20Plan%2027%20Discovery%20Trust%20Display%20Adjacency%20and%20Name%20Quality.md), written 2026-07-28.** Read Plan 27's validated facts before acting on the notes here: three of them are now known to be wrong or incomplete. **F-26-11's fourth display-only state is retired** (W1.4 closed it — every streamed item is a persisted row), so F-26-42 is smaller than described. **The claim that "the browse-card half shipped in W1.4" is false** — the trust label has only ever lived in the Details panel, and the component's own tests pin that. And **Q-26-3's supporting statistics do not survive a base-rate check**: bracket-stripping is already implemented and already failing at scale, the "182 six-plus-word names" figure is an artifact (long names are slightly *more* common among verified rows), and the real lookup wins come through aliases. Plan 27 answers Q-26-3 **no** as a schema change and **yes** as a prompt fix.
