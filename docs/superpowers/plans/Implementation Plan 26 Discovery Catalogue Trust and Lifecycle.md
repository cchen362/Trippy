...
**Status:** 2026-07-28 — **W1, W2, W3 and the F-26-23 follow-on COMPLETE, DEPLOYED and production-QA'd (`4921dd2`). W4 COMPLETE, DEPLOYED (`f0aeabe`) and production-QA'd — Appendix D PASSED with no defects and no QA debt (results and F-26-36/F-26-37 in **Appendix E**). W5 is the only remaining wave.** The W1–W3 QA passed sections A–D after two defects in the W3.3 operator script were found by section D and fixed — full results, and four new facts (F-26-24…F-26-26), in **Appendix C**. W4 closed the creation path that F-26-26 caught live: the post-deploy invariant is that production still holds **exactly three** empty-country destinations, and a fourth would mean W4.5 has a hole.
...
**F-26-37 — the accepted cost is now visible in production, and this case is one where W4 traded a silently-correct outcome for a click.** Pre-W4, `resolveCountryForCityText('乌镇')` would have weak-matched (`estimated`/0.55, the CJK-name class in F-26-32), stored `CN`, and generated a **correct** `乌镇|CN` catalogue with no prompt. Post-W4 the same action costs one confirmation. **This is not a regression and needs no fix** — it is exactly the F-26-32/F-26-33 finding that no threshold on `locationStatus` separates 乌镇-correct from Georgetown-wrong or the `qwxzptlkvv`→"Equinix Singapore" junk row. The easy cases pay a click so the dangerous ones become impossible. Recorded because it is the first production instance and will otherwise be re-litigated as "W4 made this worse".
