...
- **Design decisions made during implementation:**
  ...
  - `AddBookingModal` (untouched, per scope) only renders field blocks for `hotel`/`flight`/`train`/`other` and locks its type selector in edit mode. Extracted `bus`/`ferry` bookings are remapped to `type: 'other'` **only** on the object passed into the editor (`remapForEditing()` in `CaptureFlow.jsx`); the card's own badge and the unedited confirm payload keep the true type.
