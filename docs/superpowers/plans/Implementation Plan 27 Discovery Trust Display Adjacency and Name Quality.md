...
**Status:** 2026-07-28 — **WRITTEN. NOT STARTED.** Four waves, **no migration in the entire plan** (see F-27-1). All four owner decisions are taken (D-27-1…D-27-4); no wave is blocked on a product question. Investigation was read-only: no product code was written in the session that produced this document.
...
**Status:** 2026-07-28 — **NOT STARTED.** Presentation-only, frontend-only, no migration, no new external call, no server change. Lowest risk in the plan; ship it first.
