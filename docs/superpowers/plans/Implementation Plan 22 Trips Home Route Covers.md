...
`BottomNav.jsx:6` stacks two dimming sources on one element: `color: var(--cream-mute)` (`index.css:17` = `rgba(240,234,216,0.28)`) **and** `opacity: 0.45`. Effective label opacity = `0.28 × 0.45 ≈ 0.126` — about 4.8× fainter than a normal inactive tab (`--cream-dim` = 0.60, `:17`/`BottomNav.jsx:17`). On Trips Home, `<BottomNav />` mounts with no props → `inTrip = false` → Plan/Logistics/Map all disabled (`:43-45`), so the page shows one legible tab beside three ghost labels that read as broken. Owner decision: **omit** them entirely rather than restyle (D9).
...
- `frontend/src/index.css` — tokens: `--cream-dim` 0.60 `:16`, `--cream-mute` 0.28 `:17`, `--ink-border` `:11`, `--gold` `:12`, `--gold-soft` `:13`, `--gold-line` `:14`; `.trip-card-overlay` `100deg` gradient `:59-60`. Reuse tokens; do not introduce new ones.
...
- **Token scope (owner steer):** owner approved using the revamped design system's tokens (`docs/superpowers/mockups/trippy-revamped-system.css`), but the running app only loads `index.css`. Added **only** the two new tokens W2 consumes — `--ink-border-strong`, `--ink-satin` (plus `--shadow-deep`, `--radius-l` already present as W2 material) — additively; did **not** flip shared token values (`--cream-dim`/`--gold-line`/etc.), which would restyle every screen. Dropped `--ink-raised` (unused this wave). Full revamp-token reconciliation + CLAUDE.md update spun off as a separate focused session (owner's request).
