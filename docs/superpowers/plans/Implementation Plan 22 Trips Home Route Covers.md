...
**Deployed 2026-07-22** — `main` → `8edc34f` pulled on the server, `docker compose up -d --build`, container `trippy-trippy-1` recreated and healthy (`{"status":"ok","db":"connected"}`, `:6768`). No migration ran (schema impact NONE). Pre-deploy consistent `sqlite3 .backup` snapshot taken to `~/Trippy/backups/trippy.db.pre-6ae51ab-predeploy-8edc34f-*.bak`. Prod-served bundle hash `index-C-FW6pVU.js` matches the local build and contains `destinationsGeo` + `Active now`. **Plan stays OPEN pending owner prod QA** (phone-width click-script handed to owner in chat); close only after owner passes.
