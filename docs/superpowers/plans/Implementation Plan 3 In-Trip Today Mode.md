...
- [x] H1 *(server)* Re-auth Tailscale on the Debian box; deploy current `main` (booking capture reaches production for the first time). Verify `trippy.zyroi.com` shows the "+ Add bookings" capture flow. **Done 2026-07-05** — deployed commit `00cc959` (production was 4 weeks behind at `e870b6e`). Pre-deploy backup taken first (`~/backups/trippy-pre-h1-2026-07-05.db`, 385KB, verified restorable). `docker-compose build && docker-compose up -d`; migrations 011 (`import_artifacts`) and 012 (`booking_attachments`) applied cleanly on startup, no errors, container stable. Verified via curl: home page 200, `/api/auth/me` 401 (expected, unauthenticated), share-link token still resolves 200 (checked without ever printing the token). Trips-home/capture-flow/Map-pins visual checks handed to the user — the session's Chrome browser-automation tool was unreachable, and entering a password to log in is off-limits for Claude regardless.
- [x] H2 *(server)* Nightly backup cron on the Debian host: `sqlite3 <data>/trippy.db ".backup <backups>/trippy-$(date +%F).db"` + 14-day rotation, backups on local disk per user decision (documented caveat: does not survive disk failure). Verify a backup file restores and opens. **Done 2026-07-05** — script at `~/trippy-backup.sh` (host, outside the git-tracked repo dir so it isn't touched by future deploys), cron `0 3 * * *` appended alongside the pre-existing unrelated `aihtml` entry (not replaced). Ran once manually: produced `~/backups/trippy-2026-07-05.db` (430KB), verified it opens and `SELECT COUNT(*) FROM trips` returns 2.
...
**M0 status:** All of H3–H8 and H1–H2 shipped and verified. H1/H2 done 2026-07-05 (deployed `00cc959`, nightly backup cron installed) — see notes above. M0 is fully complete.
...
- [x] `Status` pill on flight heroes → existing flights lookup, render live fields + "checked HH:MM"; absent for non-flights; graceful degrade to deep link on provider failure (D6). **Done 2026-07-04**, revised same day after a UX follow-up — `backend/src/services/lookups.js`'s `normalizeAeroDataBoxFlight` now also extracts `status`/`departureTerminal`/`departureGate`/`arrivalTerminal`/`arrivalGate`/`revisedDeparture`/`revisedArrival` (all optional, per the Risks note on live-field coverage). `frontend/src/components/today/StatusPill.jsx` fetches once, silently, the moment the flight hero mounts (`useEffect` keyed on `booking.id`) instead of waiting for a tap — reasoned against the project's API Cost Discipline: a single fetch-on-view stays well inside AeroDataBox's rate limits, whereas true real-time polling would run a background timer for as long as the hero is on screen and conflicts with D6's "no polling" rule and the system-of-reference principle. The pill doubles as a manual refresh (tap again, a small `RefreshCw` icon signals this) for a fresh check right before boarding. Reads `booking.detailsJson.carrierCode`/`.flightNumber` the same way `FlightBookingCard.jsx` already does. Non-flight heroes render nothing (unchanged `showStatus` gate in `HeroCard.jsx`); provider failure/`manual_only` shows a plain "Status unavailable" pill (still tappable to retry) rather than blocking the card.
- [x] TripsHomePage: active-trip card deep-links to `/today`. **Done 2026-07-04** — `TripCard.jsx` now branches on `tripIsLive(trip)` (the same M2 helper `TripPage.jsx` uses) to pick `/today` vs `/plan`.
- [x] PWA runtime cache for artifact/attachment GET responses (CacheFirst, they're immutable) so tickets open offline. **Done 2026-07-04** — two new `CacheFirst` rules in `vite.config.js` (`trippy-artifact-files`, `trippy-booking-attachments`), matching `resolveBookingDocuments`'s exact URL shapes; `DocumentViewer.jsx`'s plain `<img>`/`<embed>` tags are intercepted transparently, cookies included as with the uncached case.
- Verify: **Done 2026-07-04** — backend: extended `lookups.test.js`'s AeroDataBox fixture with live fields plus a fields-absent case (176 backend tests pass); frontend build and vitest (12 tests) both clean after each revision. Live-verified in the browser (system date patched to fall inside the Ipoh–Kuala Lumpur trip's real range): the flight hero auto-loaded "Departed · Gate D44 · Terminal 1" + a "checked HH:MM" stamp from the real AeroDataBox response with no tap, and a manual re-tap refreshed it again successfully; a train hero showed no pill at all (flight-only gate holds); the active-trip card on `TripsHomePage` linked to `/today` while live and `/plan` once patched back to the real (non-live) date. PWA `CacheFirst` rules verified by inspecting the built `dist/sw.js`, which compiled both new rules correctly — a full airplane-mode round trip wasn't run because `vite preview`'s origin isn't covered by the backend's locked-down CORS config (`origin: config.frontendUrl`) and the dev server doesn't register a service worker at all, so simulating it would have meant standing up throwaway proxy config rather than testing the shipped path.

**M3 status:** Shipped. All three tasks additive, no schema changes. H1/H2 completed 2026-07-05 (see M0). This closes the entire In-Trip Mode plan (M0–M3) — no open items remain.
