...
- [ ] `tripIsLive` helper + BottomNav swap + TripPage index redirect + `/today` route with off-day redirect (D3).
- [ ] Anchor derivation module (`frontend/src/utils/todayModel.js` or similar): pure function `(days, bookings, now) → { collapsed, hero, upcoming, tonight, tomorrowFirst }` implementing D2 — unit-testable without UI; cover: morning-activities-before-train collapse at departure, hotel-fallback hero, free day, overnight/cross-midnight bookings, booking hidden from itinerary still anchoring by time.
- [ ] TodayTab page per agreed mockup: day header ("Thu · day 4 of 7", city in Playfair italic), collapsed-earlier row, hero card (gold label = the component's single gold accent; `Navigate` / `Ticket` / `Status` pills), quiet later-today rows, Tonight slot (hotel + check-in ref + navigate), tomorrow-preview footer ("Tomorrow starts at 09:15 — …" from tomorrow's first anchor; omitted when none).
- [ ] Shared deep-link util (D7); navigate icons only where coordinates exist.
- Verify on 375px viewport with device clock mid-trip: correct hero, collapse behavior at an anchor's passing, Ticket opens the vault viewer, tab absent outside trip dates.
