...
**M3 status:** Shipped. All three tasks additive, no schema changes. This closes the In-Trip Mode plan (M0–M3) — H1/H2 (Debian server deploy + backup cron) remain the only open items, blocked on the Tailscale re-auth noted in M0.
