...
**Status: CLOSED 2026-07-20 — deployed (728c7f2 + hotfix c87b639), owner production click-script QA fully passed (10/10 after the #6/#7 hotfix re-check: owed-name field renders, booking link persists with glyph).** Root causes: `Number()` coercion on the Linked-booking and Paid-by `<select>`s NaN'd the hex-string ids (selection silently dropped from the payload); `.modal-input`'s `width:100%` beat the `w-28` utility on the owed amount input, stretching it and collapsing the flex-1 name field to a sliver (the stray "Dw" name was Chrome autofill landing in that sliver). Fix verified in a real browser pre-deploy: owed row renders wide-Name + narrow-amount, booking selection persists (`booking_id` confirmed in DB, glyph renders). Deploy verified on server: migration 031 applied cleanly at startup (`_migrations` id 31, expenses/expense_owed/fx_rates tables + trips.summary_currency present, expenses empty), health OK on :6768, SPA serving new bundle, expenses route mounted (401 unauthenticated), container up, pre-deploy DB backup `trippy.db.pre-728c7f2.bak` (+ -wal/-shm) taken. W1: migration 031, currency utils, fx.js, expenses service/routes, trips summaryCurrency — 646/646 backend tests, migration proven on a real-DB copy. W2: expensesApi + useExpenses, wallet header icon (leftmost of 4), Expenses view, ModalShell capture sheet, summary-currency prompt — 178/178 frontend tests, clean build. Orchestrator smoke pass 2026-07-20 at 375px: create-expense end-to-end, CNY day-geo currency default, totals + D5e unestimated note + D5 footer render; FX service verified against live CDN for a past date (today's snapshot not yet published → correctly unestimated, never guessed). W3 scope extended 2026-07-20 with owner-approved FX robustness items (§3 W3.5: dedupe, negative cache, bounded same-request stamping, silent refetch).
**Date:** 2026-07-19
...
5. **FX robustness extras (owner-approved 2026-07-20, added post-W1/W2):** in `backend/src/services/fx.js`, (a) in-flight dedupe — concurrent `getRate` calls for the same (pair, date) share one promise/fetch; (b) short negative cache — after a total miss for a (pair, date), skip network retries for ~15 minutes (in-memory is fine; failures must still never persist to `fx_rates`). In `backend/src/services/expenses.js` list read: (c) bounded same-request stamping — when unestimated rows exist, await their stamp attempts under one ~700ms overall `Promise.race` budget so healed rows return estimated in the same response; stragglers stay unestimated (D5a still holds: saves never wait, reads wait at most the fixed budget). In `frontend/src/hooks/useExpenses.js`: (d) one silent refetch ~3s after a list response containing unestimated rows. Cover (a)–(c) in backend tests with mocked fetch; exercise (b)+(c) in the W3.3 failure drills.
6. Owner click-script for production QA post-deploy (per standing practice); update this plan's status line; commit.

**W3 COMPLETE 2026-07-20.** W3.5 robustness shipped: fx.js in-flight dedupe (shared promise per BASE:QUOTE:date) + 15-min in-memory negative cache (failures never persisted to `fx_rates`, success clears the entry, `_resetFxMemoryForTests()` for isolation); `listExpenses` is async with one overall 700ms `Promise.race` stamping budget (healed rows re-read and totals computed post-heal; stragglers keep running uncancelled; saves still fire-and-forget); GET route awaits; `useExpenses` fires exactly one silent refetch 3s after a non-retry list response containing an unestimated row (retry never re-arms — no polling loop, verified in-browser: exactly 1 extra list call, none after). Suites: backend 650/650 (4 new: dedupe single-fetch, negative-cache no-refetch + empty `fx_rates`, same-response heal, slower-than-budget stays null), frontend 178/178, build clean. **Live FX drills** (real CDN, temp DB): TWD→SGD 0.03984472 and VND→SGD 0.000049176505 @2026-07-18 via jsDelivr; repeat = 0 fetches, one row per pair-date; forced-primary-failure → pages.dev fallback returned CNY→SGD 0.19059054; 3 concurrent calls = 1 fetch; total outage → null, second call 0 fetches, `fx_rates` untouched. **Browser pass** (real Chrome, 375px + desktop): ≤4-tap capture (¥45); dated 2026-07-18 entry with CNY chip day-default, chip→TWD one-tap change, booking link (Park Hyatt Hangzhou), Sarah-owes-¥80 → create response correctly unestimated (save never waits), next list response healed in-request (¥200→S$38.12 est., NT$500→S$19.92 est.; Spent S$58.04, awaiting S$15.25, exact vs. hand math); settle toggle → awaiting S$0.00; multi-currency unestimated note ("+ NT$500.00 unestimated, + ¥165.00 unestimated" while today's CDN snapshot unpublished — D5e by design); D5 footer sentence + "est." suffix verbatim. **Failure drills:** owed 1500>1000 rejected in UI (inline error, entry preserved) and API (HTTP 400); offline save → loud "Something went wrong / Failed to fetch" banner, no queue, form intact; Save/Cancel footer pinned and reachable at 375×500 (keyboard-height) viewport. **Share regression** (running server): minted share link for the expense-bearing trip → 7.6KB public payload, zero matches for expense/owed/fx/amount/name fields; link deleted after. Deferred to owner (below): real-device motion feel + software keyboard. NOT deployed.

### Owner production click-script (run after `/deploy`)

Phone (or 375px window), production URL, signed in as owner:

1. Open the Shanghai–Hangzhou trip → tap the **wallet icon** (leftmost of the four header icons). Expect the Expenses view; title may truncate but ← Trips and all four icons stay tappable.
2. First open of a trip with no summary currency prompts once for it (default SGD). Pick and confirm it doesn't re-prompt.
3. **Fast capture:** + Add Expense → type an amount → tap a category → Save. Four taps, sheet closes, entry appears instantly. With the phone keyboard open, confirm Save is visible/tappable (not behind the keyboard).
4. **Currency + date:** new expense → tap the currency chip (should pre-fill from that day's country) → pick another currency; set Date to a past date. Save. The entry may briefly show "unestimated", then gain an "est." converted amount within ~3s or on next open — daily-mid-market footer sentence should read exactly: "Estimates use daily mid-market reference rates, not your card's exchange rate."
5. A **today-dated** foreign-currency entry may stay "unestimated" until tomorrow (the provider publishes daily snapshots) — that plus a "+ … unestimated" note in the summary is correct, not a bug.
6. **Owed flow:** new expense → More → Add person → name + amount (try an amount larger than the expense first: expect an inline rejection). Save with a valid amount → the person appears under Open repayments → Mark settled → Awaiting repayment drops.
7. **Booking link:** new expense → More → pick a booking under Linked booking → save → entry shows the booking glyph.
8. Airplane mode → try to save an expense → expect a loud inline error with your input preserved (no silent queue). Disable airplane mode, Save again → succeeds.
9. Open the trip's **public share link** in a private window → confirm nothing about expenses, amounts, or names appears anywhere.
10. Motion feel: sheet slide-up/dismiss and settle-toggle feedback at your judgment (agent verified end states, not feel).
