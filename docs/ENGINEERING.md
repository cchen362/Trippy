...
- `/mcp` is a hosted Model Context Protocol endpoint served **in-process** by the same Express app (`backend/src/routes/mcp.js`, `backend/src/services/mcp/`), never a sidecar: provider daily budgets are in-memory per process (`config.js`), so a second process would double every quota. It is flag-gated by `MCP_ENABLED=1` and needs `MCP_PUBLIC_URL` (the exact public `/mcp` URL) — with the flag off nothing is mounted and `/api/health` reports `mcp.enabled: false`.
- **Mount order is load-bearing (F-28-22).** `createMcpRouter` is mounted *before* the global `express.json({ limit: '16mb' })` in `index.js` because `POST /mcp` carries its own 1 MB JSON parser and `PUT /mcp/uploads/:ticket` its own `express.raw` — the global parser would claim (or corrupt) those bodies first. Do not reorder the mounts in `index.js`.
- **Auth is personal integration tokens, not OAuth (D-28-1).** A token is minted once in the Account modal → Integrations, shown once, and stored only as a sha256 hash (`integration_tokens`, migration 033); revoke is instant, delete is a separate cleanup allowed only on revoked rows (W5.6). Three coarse scopes (D-28-6): `trips:read`, `trips:write`, `documents:write`. Scope failures are `isError` tool results with `error: 'insufficient_scope'`, not HTTP 403. Claude.ai / Claude Desktop connectors require OAuth and therefore cannot connect; W4 (OAuth) is not planned.
- **Seven tools, no more:** `list_trips`, `get_trip`, `prepare_draft`, `apply_draft`, `get_apply_status`, `request_upload_ticket`, `prepare_delete`. Every write goes preview → `mcp_drafts` row (30-minute TTL, D-28-4) → explicit `apply_draft`, all-or-nothing per draft (D-28-3), fingerprint-guarded against a trip that changed in between. `prepare_delete` → `apply_draft` is the **only** destructive path (D-28-8). A new trip's title/dates/destinations always come from the client — Trippy never infers a date range from a booking (D-28-5).
- **Never add a model call on the MCP path.** The client interprets the screenshot/PDF/email; Trippy validates structured fields with the same services the REST API uses. Anthropic is not called for any MCP tool, and that is a cost boundary, not an omission.
- **Streaming and shutdown.** `apply_draft` emits `notifications/progress` (or `notifications/message`) before its first provider call so Cloudflare's 100 s no-bytes timeout never fires (proven at +153 s through the public host). `index.js` keeps the `http.Server` and handles `SIGTERM`/`SIGINT` through `backend/src/shutdown.js`: stop accepting → drain idle → destroy in-flight (the SDK aborts the request signal, and `apply.js` throws `cancelled` before its transaction) → close the DB. A draft interrupted mid-resolve stays `pending` and a retry applies cleanly.
- **Logs are secret-free.** Every tool call writes one line via `services/mcp/log.js` — timestamp, token prefix, user id, tool, duration, outcome, draft id — never tool arguments, response bytes, booking fields, or the plaintext token.
- Uploads: `apply_draft` (or `request_upload_ticket`) issues a one-time 15-minute ticket; the client `PUT`s raw bytes to `/mcp/uploads/:ticket` with no bearer (the ticket id is the credential), the body's sha256 must match, and a re-upload of the same bytes is idempotent. Attachments carry `content_hash` + `source` (`'manual'` | `'mcp'`, migration 035).
- **Host compatibility contract (Plan 29).** Every result carries the structured object as a second text block (D-29-1), summaries name each issue (D-29-2), optional inputs accept `null` (D-29-3), and an empty `list_trips` names how many past trips the filter hid — the fact only, never what the host should do next (D-29-4). Never assume the host reads `structuredContent`; never assume it omits an optional rather than sending `null`.
