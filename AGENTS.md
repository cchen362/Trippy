...
## API Cost Discipline
- Codex API: discovery results live in the `discovery_destinations` / `discovery_places` catalogue (7-day TTL, keyed by `canonicalGeoKey` + country code). Never call Codex for a destination whose catalogue is still fresh.
